...
export class SecurityPolicy {
    ...
    preTransactionCheck(tx) {
        ...
        if (this.dailySpent + tx.amount > this.config.dailyLimit) {
            warnings.push(`Daily limit: spending ${this.dailySpent + tx.amount} QFC exceeds ${this.config.dailyLimit} QFC daily limit`);
            requiresConfirmation = true;
        }
        // Auto-approve small transactions to known addresses
        const approved = !requiresConfirmation &&
